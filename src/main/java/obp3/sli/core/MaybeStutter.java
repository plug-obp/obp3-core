 ...
 * @since 1.8
 */
public final class MaybeStutter<T> {
    /**
     * Common instance for {@code stutter()}.
     */
    private static final MaybeStutter<?> STUTTER = new MaybeStutter<>(null);
     ...
     */
    public static<T> MaybeStutter<T> stutter() {
        @SuppressWarnings("unchecked")
        MaybeStutter<T> t = (MaybeStutter<T>) STUTTER;
        return t;
    }
     ...
     *
     * @param action the value to describe; it's the caller's responsibility to
     *        ensure the value is non-{@code null} unless creating the singleton
     *        instance returned by {@code empty()}.
     */
    private MaybeStutter(T action) {
        this.action = action;
    }
     ...
     */
    public static <T> MaybeStutter<T> of(T value) {
        return new MaybeStutter<>(Objects.requireNonNull(value));
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> MaybeStutter<T> ofNullable(T value) {
        return value == null ? (MaybeStutter<T>) STUTTER
                : new MaybeStutter<>(value);
    }
     ...
     */
    public MaybeStutter<T> filter(Predicate<? super T> predicate) {
        Objects.requireNonNull(predicate);
        if (isStutter()) {
            return this;
        } else {
            return predicate.test(action) ? this : stutter();
        }
    }
     ...
     */
    public <U> MaybeStutter<U> map(Function<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
        if (isStutter()) {
            return stutter();
        } else {
            return MaybeStutter.ofNullable(mapper.apply(action));
        }
    }
     ...
     */
    public <U> MaybeStutter<U> flatMap(Function<? super T, ? extends MaybeStutter<? extends U>> mapper) {
        Objects.requireNonNull(mapper);
        if (isStutter()) {
            return stutter();
        } else {
            @SuppressWarnings("unchecked")
            MaybeStutter<U> r = (MaybeStutter<U>) mapper.apply(action);
     ...
     * @since 9
     */
    public MaybeStutter<T> or(Supplier<? extends MaybeStutter<? extends T>> supplier) {
        Objects.requireNonNull(supplier);
        if (isAction()) {
            return this;
        } else {
            @SuppressWarnings("unchecked")
            MaybeStutter<T> r = (MaybeStutter<T>) supplier.get();
            return Objects.requireNonNull(r);
        }
    }
    ...
    public T orElse(T other) {
        return action != null ? action : other;
    }
    ...
    public T orElseGet(Supplier<? extends T> supplier) {
        return action != null ? action : supplier.get();
    }
    ...
    public boolean equals(Object obj) {
        ...
        return obj instanceof MaybeStutter<?> other
